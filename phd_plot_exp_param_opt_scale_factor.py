...
path = r"o:\\DATA\\of_experiments\\param_optimize\\"

data = loadtxt(path + r"exp_param_opt_scale_factor.txt")
...
ax1.set_xlabel('image scale factor on coarse level', size=label_font_size)
...
#
#
y_lables = ['{:.1f}'.format(x) for x in arange(0.1, 0.6, 0.1)]
y_lables[0] = ''

#ax1.set_ylim(0.1, 0.5)

ax1.set_yticks(arange(0.1, 0.6, 0.1))
ax1.set_yticklabels(y_lables)
#
ax1.set_xticklabels(['{:.1f}'.format(x) for x in arange(0, 1.2, 0.2)])
#
